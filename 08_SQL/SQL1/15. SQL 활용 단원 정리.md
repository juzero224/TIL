...
SELECT  AVG(A.SALES_QTY * B.PRICE) AS 평균_구매금액
        , SUM(A.SALES_QTY * B.PRICE) / COUNT(ORDER_NO) AS 1회_주문시_구매금액
  FROM  SALES AS A
  LEFT
  JOIN  PRODUCT AS B
...
/***
SELECT  MEM_NO
        , SUM(A.SALES_QTY * PRICE) AS 구매금액
        , COUNT(ORDER_NO) AS 구매횟수
        , SUM(A.SALES_QTY * PRICE) / COUNT(ORDER_NO) AS 1회_주문시_구매금액
  FROM  SALES AS A
  LEFT
  JOIN  PRODUCT AS B
    ON  A.PRODUCT_CODE = B.PRODUCT_CODE
...
SELECT  MEM_NO
		, AVG(B.PRICE * A.SALES_QTY) AS 평균_구매금액
  FROM  SALES AS A
  LEFT
  JOIN  PRODUCT AS B
    ON  A.PRODUCT_CODE = B.PRODUCT_CODE
...
SELECT  MEM_NO
        ...
        , DENSE_RANK() OVER (ORDER BY SUM(SALES_QTY) DESC) AS 동일한_순위_하나의_등수
  FROM  SALES
...
DELIMITER //
CREATE PROCEDURE CST_BIRTHDAY(IN INPUT_A INT, INPUT_B INT)
BEGIN
	SELECT  *
      FROM  CUSTOMER
